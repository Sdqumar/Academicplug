...
function AddCourse({School, Faculty, Department}) {
    ...
    const validationSchema = Yup.object().shape({Course: Yup.mixed().required('Required')});
    ...
    const onSubmit = async (values, actions) => {
        ...
        await setDoc(doc(firestore, 'schools', School, 'courses', Course.replace(/\s/g, '-')), {
            Course,
            School,
            Faculty,
            Department,
							...
							(formik)=>{
								return(
                                    ...
                                    <Form>
                                        <Box width="15rem"
                                            mt={2}>
                                            <FormikControl control="chakraInput" type="name" label="Course Name" name="Course"/>
                                        </Box>
                                    </Form>
									<Box>
                                    <UploadInput getfile={getfile}
                                        label="pdf"
                                        size={
                                            {
                                                byte: '10000000',
